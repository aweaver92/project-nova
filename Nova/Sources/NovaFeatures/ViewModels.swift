...
public final class SessionViewModel {
    ...
    public func register() async {
        ...
        do {
            ...
            // Success path (including "already registered" treated as connected)
            // clears any prior failure banner.
            errorMessage = nil
...
public enum StudyDeepLink: Equatable, Sendable {
    case open
    case startReview(deck: String?)
}

public struct StudyDeckSummary: Identifiable, Equatable, Sendable {
...
public final class StudyViewModel {
    ...
    public private(set) var reviewDeckFilter: String?

    /// HUD / Agents deep-link: open Study or start a review after tab switch.
    public var pendingDeepLink: StudyDeepLink?
    /// Agents tab sets this so NavigationLink can present StudyView.
    public var shouldPresentStudy = false

    public var hasDecks: Bool { !deckSummaries.isEmpty }

    public var currentReviewCard: StudyCard? {
        guard isReviewing, reviewIndex >= 0, reviewIndex < reviewQueue.count else { return nil }
        return reviewQueue[reviewIndex]
    }

    public var reviewProgressLabel: String {
        guard isReviewing, !reviewQueue.isEmpty else { return "" }
        return "\(min(reviewIndex + 1, reviewQueue.count)) / \(reviewQueue.count)"
    }
    ...
    public func load() async {
        ...
        dueTotal = due.count
        let dueByDeck = Dictionary(grouping: due, by: \.deck)
        let allByDeck = Dictionary(grouping: all, by: \.deck)
        let names = await store.decks()
        deckSummaries = names.map { name in
            StudyDeckSummary(
                name: name,
                totalCount: allByDeck[name]?.count ?? 0,
                dueCount: dueByDeck[name]?.count ?? 0
            )
        }
        if let selected = selectedDeckName {
            selectedDeckCards = all
                .filter { $0.deck.localizedCaseInsensitiveCompare(selected) == .orderedSame }
    ...
    public func deleteCard(id: UUID) async {
        ...
        if currentReviewCard?.id == id {
            advanceAfterGrade()
        }
        await load()
    }

    public func startReview(deck: String? = nil, limit: Int = 50) async {
        let due = await store.due(deck: deck, limit: limit)
        reviewQueue = due
        reviewIndex = 0
        isRevealed = false
        isReviewing = !due.isEmpty
        reviewDeckFilter = deck
        await load()
    }
    ...
    public func endReview() {
        ...
        reviewQueue = []
        reviewIndex = 0
        reviewDeckFilter = nil
    }

    /// Called from Assistant HUD — switches into Agents Study surface.
    public func requestOpenStudy() {
        pendingDeepLink = .open
        shouldPresentStudy = true
    }

    public func requestStartReview(deck: String? = nil) {
        pendingDeepLink = .startReview(deck: deck)
        shouldPresentStudy = true
    }

    /// Consume deep link once StudyView appears.
