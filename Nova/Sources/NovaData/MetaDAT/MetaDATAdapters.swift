...
/// Camera stills / low-FPS live look. Replace internals with `MWDATCamera` stream APIs.
public actor MetaDATFrameCapture: FrameCapture {
    ...
    /// Call when conversational audio is under pressure — drops live look.
    ...
    public func captureStill() async throws -> CapturedFrame {
        // Production: request photo from active DAT stream / capture API.
        // Placeholder 1x1 JPEG header-ish bytes for wiring tests.
        let placeholder = Data([0xFF, 0xD8, 0xFF, 0xD9])
        NovaLog.vision.info("captureStill (adapter stub)")
