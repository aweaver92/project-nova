...
/// File-backed store for saved voice recordings.
///
/// Audio files and their metadata index live under `Documents/Recordings`, so the
/// recordings persist on the iPhone and — with `UIFileSharingEnabled` /
/// `LSSupportsOpeningDocumentsInPlace` set — are visible and exportable from the
/// Files app.
public actor FileRecordingStore: RecordingStoring {
    private let dir: URL
    private let indexURL: URL
    private var items: [VoiceRecording]
    ...
    @discardableResult
    public func register(_ recording: VoiceRecording) -> VoiceRecording {
        items.append(recording)
        persist()
        return recording
    }

    public func all() -> [VoiceRecording] { items }
    ...
    private func persist() {
        do {
            ...
            NovaLog.session.error("Recording index persist failed: \(String(describing: error), privacy: .public)")
        }
    }

    private static func load(from url: URL) -> [VoiceRecording] {
